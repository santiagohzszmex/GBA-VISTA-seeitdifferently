        ...
        {loading ? <p className="py-16 text-center text-[#86868b]">Cargando avisos...</p> : notifications.length > 0 ? (
          <div className="divide-y divide-[#d2d2d7]/60 border-y border-[#d2d2d7]/60">
            {notifications.map(notification => {
              const NotificationIcon = iconFor(notification);
              return (
              <button
                key={notification.id}
                type="button"
              ...
              >
                <span className={`w-11 h-11 rounded-xl flex items-center justify-center flex-shrink-0 ${notification.leida ? 'bg-[#f5f5f7] text-[#86868b]' : 'bg-blue-50 text-[#0066FF]'}`}><NotificationIcon size={18}/></span>
