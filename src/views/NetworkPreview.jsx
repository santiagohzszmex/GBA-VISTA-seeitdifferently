...
import networkCapital from '../assets/network-capital.webp';
import {
  NETWORK_CATEGORIES,
...
const mapBusiness = (business, index) => ({
  ...
  kind: business.account_type === 'company' ? 'Empresa' : 'Negocio',
...
function DetailModal({ item, onClose }) {
  ...
  return (
    <div className="net-overlay"><div className="net-modal max-w-2xl"><header className="net-modal-header"><div><p className="net-eyebrow">{isOpportunity ? item.type : item.kind}</p><h3 className="font-bold mt-1">{isOpportunity ? item.title : item.name}</h3></div><button type="button" className="net-icon" title="Cerrar" onClick={onClose}><X size={16}/></button></header><div className="p-5 md:p-7"><div className="flex items-center gap-3"><IdentityMark item={item} large/><div><strong className="block text-base">{isOpportunity ? item.organization : item.headline}</strong><span className="flex items-center gap-1 text-[10px] text-[#8b9099] mt-1"><MapPin size={11}/>{item.location || 'Empyria'}</span></div></div><p className="text-sm leading-6 text-[#626770] mt-6">{isOpportunity ? item.summary : item.description}</p><div className="flex flex-wrap gap-2 mt-6 pt-5 border-t border-[#e5e7eb]">{(isOpportunity ? item.areas : item.tags).map(tag => <span key={tag} className="h-7 px-2.5 rounded bg-[#f1f3f6] text-[9px] font-bold text-[#686d75] flex items-center">{tag}</span>)}</div>{item.contact && <div className="mt-6 px-4 py-3 border-l-2 border-[#2563eb] bg-[#f4f7fd]"><span className="net-label">Contacto aprobado</span><strong className="text-sm">{item.contact}</strong></div>}</div></div></div>
  );
}
...
export default function NetworkPreview({ previewMode = false, onOpenStudio }) {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      setLoading(true);
      const { data, error: requestError } = await supabase.from('network_businesses').select('*').eq('estado', 'aprobado').order('promocionada', { ascending: false }).order('updated_at', { ascending: false });
      if (!active) return;
      if (requestError) {
  ...
  return (
    <div className="min-h-screen bg-[#f5f6f8] text-[#202328] pb-24 md:pb-16"><style>{NETWORK_STYLES}</style>
      ...
      <section className="relative min-h-[350px] md:min-h-[430px] overflow-hidden bg-[#1b1c1f]"><img src={networkCapital} alt="Ciudad construida dentro de Empyria" className="absolute inset-0 w-full h-full object-cover"/><div className="absolute inset-0 bg-black/48"/><div className="relative max-w-[1440px] mx-auto px-5 md:px-10 py-12 text-white flex items-end min-h-[350px] md:min-h-[430px]"><div className="max-w-2xl"><BetaBadge dark/><h2 className="text-3xl md:text-5xl font-black mt-5 leading-tight">Todo lo que se esta creando en Empyria.</h2><p className="text-sm md:text-base text-white/80 leading-6 mt-4 max-w-xl">Descubre negocios, proyectos, equipos creativos y convocatorias que viven dentro de la comunidad.</p><button type="button" className="net-button bg-white text-[#17191d] hover:bg-[#f1f1f1] mt-7" onClick={() => setView('opportunities')}><UserRoundSearch size={14}/>Ver oportunidades</button></div></div></section>
